...
export default function RegisterPopup({ closeFunction }) {
  ...
  return (
    <Overlay>
      ...
      <div className={`fixed z-1 px-15 py-10 p border-black bg-bgsecondary top-1/4      flex flex-col gap-4 text-white rounded-sm justify-between h-110
      ...
      ${mounted ? "scale-100 opacity-100" : "scale-80 opacity-0"}`}>
        ...
        <button className="self-start border-gray-500 border 1 bg-green-500 px-5 py-1 rounded-sm mt-1 text-white font-semibold opacity-90 hover:opacity-100">
          Sign Up
        </button>
      </div>
    </Overlay>)
